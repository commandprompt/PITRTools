...
CREATE OR REPLACE FUNCTION cmd_get_data_dirs() RETURNS SETOF TEXT AS $$
   SELECT DISTINCT pg_tablespace_location(oid) as spclocation from pg_tablespace WHERE pg_tablespace_location(oid) <> '';
$$ LANGUAGE SQL;
...
CREATE OR REPLACE FUNCTION cmd_get_tablespaces() RETURNS SETOF TEXT AS $$
        SELECT DISTINCT pg_tablespace_location(oid) as spclocation from pg_tablespace WHERE pg_tablespace_location(oid) IS NOT NULL;
$$ LANGUAGE SQL STABLE;
